...
async function loadModelWithRetry(maxRetries = CONFIG.maxRetries) {
    ...
    while (attempt < maxRetries) {
        try {
            ...
            // Configurar el modelo con la forma de entrada correcta
            const modelConfig = {
                strict: false, // Permitir cierta flexibilidad en la carga
            };
            ...
            // Verificar y ajustar la capa de entrada
            if (loadedModel.layers && loadedModel.layers.length > 0) {
                const inputShape = [null, CONFIG.inputShape.height, CONFIG.inputShape.width, CONFIG.inputShape.channels];
                loadedModel.layers[0].batchInputShape = inputShape;
            }
            
            // Calentar el modelo con las dimensiones correctas
            const dummyInput = tf.zeros([1, CONFIG.inputShape.height, CONFIG.inputShape.width, CONFIG.inputShape.channels]);
            const warmupResult = await loadedModel.predict(dummyInput).data();
            dummyInput.dispose();
