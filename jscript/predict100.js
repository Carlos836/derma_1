...
async function loadModelWithRetry(maxRetries = CONFIG.maxRetries) {
    ...
    while (attempt < maxRetries) {
        try {
            ...
            // Verificar y ajustar la capa de entrada si es necesario
            if (!loadedModel.inputs || loadedModel.inputs.length === 0) {
                ...
                const inputLayer = tf.layers.input({ batchInputShape: [null, ...inputShape] });
                loadedModel.layers.unshift(inputLayer);
                loadedModel.build([null, ...inputShape]);
            }
